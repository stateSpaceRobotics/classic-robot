...
def callback(msg):
    global t
    rospy.loginfo("Received a /bool message!")
    t = msg
...
def findHIDDevice(mbed_vendor_id, mbed_product_id):
    global t
    rospy.init_node('linear_bool_listener', anonymous=True)
    rospy.Subscriber("/linear_bool", bool, callback)
    rospy.Publisher("/shovel", int, queue_size=10)
